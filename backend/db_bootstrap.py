...
async def ensure_database_schema(db_pool: aiomysql.Pool) -> None:
    async with db_pool.acquire() as conn:
        ...
        async with conn.cursor() as cur:
            await cur.execute(
                ...
                CREATE TABLE IF NOT EXISTS users (
                    ...
                    profile_trader_id VARCHAR(64) NULL,
            ...
            await cur.execute(
                ...
                CREATE TABLE IF NOT EXISTS admin_support_links (
                    ...
                    quiz_intro_video_enabled TINYINT(1) NOT NULL DEFAULT 1,
            ...
            await cur.execute(
                ...
                CREATE TABLE IF NOT EXISTS user_mode_access (
                    ...
                    is_enabled TINYINT(1) NOT NULL DEFAULT 0,
                    override_mode VARCHAR(16) NOT NULL DEFAULT 'inherit',
                    updated_by BIGINT NULL,
                    updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
        ...
        await _ensure_column(conn, db_name, "users", "profile_edit_allowed", "ALTER TABLE users ADD COLUMN profile_edit_allowed TINYINT(1) NOT NULL DEFAULT 0 AFTER trader_id")
        await _ensure_column(conn, db_name, "users", "profile_name", "ALTER TABLE users ADD COLUMN profile_name VARCHAR(80) NULL AFTER profile_edit_allowed")
        ...
        await _ensure_column(
            ...
            "user_mode_access",
            "override_mode",
            "ALTER TABLE user_mode_access ADD COLUMN override_mode VARCHAR(16) NOT NULL DEFAULT 'inherit' AFTER is_enabled",
        )
        async with conn.cursor() as cur:
            await cur.execute(
                "UPDATE user_mode_access SET override_mode = 'allow' WHERE is_enabled = 1 AND override_mode = 'inherit'"
            )
        ...
        await _ensure_index(conn, db_name, "users", "idx_users_chatterfy_bot_lead_id", "CREATE INDEX idx_users_chatterfy_bot_lead_id ON users(chatterfy_bot_lead_id)")
        ...
        await _ensure_index(conn, db_name, "pocket_postback_events", "idx_pocket_postback_provider_event", "CREATE INDEX idx_pocket_postback_provider_event ON pocket_postback_events(provider_event_id)")
        await _ensure_index(
            conn,
            db_name,
            "preset_indicators",
        ...
        async with conn.cursor() as cur:
            ...
            await cur.execute(
                ...
                ON DUPLICATE KEY UPDATE
                    ...
                    quiz_intro_video_enabled = COALESCE(quiz_intro_video_enabled, VALUES(quiz_intro_video_enabled)),
